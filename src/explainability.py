...
class ModelExplainer:
    ...
    def explain_global(
        self, X_train: pd.DataFrame, y_train: Optional[pd.Series] = None, method: str = "shap"
    ) -> ExplanationResult:
        """
    ...
    def _explain_global_permutation(
        ...
        return ExplanationResult(
            ...
            visualizations=[fig],
            confidence_scores={
                feat: std for feat, std in zip(self.feature_names, result.importances_std)
            },
        )
    ...
    def _explain_global_morris(self, X_train: pd.DataFrame) -> ExplanationResult:
        ...
        problem = {
            ...
            "bounds": [[X_train[col].min(), X_train[col].max()] for col in self.feature_names],
        }

        # Generate samples
        param_values = morris_sampler.sample(problem, N=100, num_levels=4, optimal_trajectories=10)

        # Evaluate model
        Y = np.array(
            [self.model.predict(param_values[i : i + 1])[0] for i in range(len(param_values))]
        )
    ...
    def explain_local(
        self, X: pd.DataFrame, instance_idx: int, method: str = "shap"
    ) -> ExplanationResult:
        """
    ...
    def _explain_local_shap(self, X: pd.DataFrame, instance_idx: int) -> ExplanationResult:
    ...
    def _explain_local_lime(self, X: pd.DataFrame, instance_idx: int) -> ExplanationResult:
        ...
        if hasattr(self.model, "predict_proba"):
            ...
            exp = explainer.explain_instance(
                instance, self.model.predict, num_features=len(self.feature_names)
            )
    ...
    def _explain_local_anchor(self, X: pd.DataFrame, instance_idx: int) -> ExplanationResult:
        """Generate Anchor-based local explanation."""
        # Prepare data
        categorical_idx = [
            i for i, feat in enumerate(self.feature_names) if feat in self.categorical_features
        ]
    ...
    def _explain_local_counterfactual(
        self, X: pd.DataFrame, instance_idx: int
    ) -> ExplanationResult:
        ...
        cf = CounterFactual(
            self.model.predict, shape=instance.shape, target_proba=0.5, tol=0.01, max_iter=1000
        )
    ...
    def explain_predictions_batch(
        self, X: pd.DataFrame, sample_size: int = 100, methods: List[str] = ["shap", "lime"]
    ...
    def generate_report(
        ...
        importance = global_exp.global_importance or {}
        ...
        for i, exp in enumerate(local_exps.get("shap", [])[:3]):
            if exp.local_explanations:
                ...
                for feat, contrib in sorted(
                    local["feature_contributions"].items(), key=lambda x: abs(x[1]), reverse=True
                )[:5]:
...
class CloudCostOptimizer:
    ...
    def _init_azure_clients(self):
        ...
        if not AZURE_AVAILABLE:
            raise ImportError(
                "Azure SDK is required. Install with: pip install azure-monitor-query azure-identity"
            )
    ...
    def _init_gcp_clients(self):
        ...
        if not GCP_AVAILABLE:
            raise ImportError(
                "Google Cloud SDK is required. Install with: pip install google-cloud-monitoring google-cloud-billing"
            )
    ...
    def _collect_aws_metrics(
        ...
        metrics["memory_usage"] = (
            np.mean([point["Average"] for point in memory_response["Datapoints"]])
        ...
        metrics["network_ingress"] = (
            sum([point["Sum"] for point in network_in["Datapoints"]]) / (1024**3)
    ...
    def estimate_costs(
        self, usage_data: List[ResourceUsage], period: str = "daily"
    ) -> CostEstimate:
        """
        ...
        compute_cost = float(avg_cpu / 100 * provider_pricing["compute_per_hour"] * hours)
        memory_cost = float(avg_memory * provider_pricing["memory_per_gb_hour"] * hours)
        storage_cost = float(
            total_storage * provider_pricing["storage_per_gb_month"] / 30 * (hours / 24)
        )
        network_cost = float(total_network * provider_pricing["network_per_gb"])

        total_cost = float(compute_cost + memory_cost + storage_cost + network_cost)
        cost_per_prediction = float(total_cost / max(total_requests, 1))
    ...
    def optimize_resources(
        ...
        cpu_percentiles = np.percentile([u.cpu_usage for u in usage_history], [50, 75, 90, 95])
        # Memory percentiles reserved for future optimization
        _ = np.percentile([u.memory_usage for u in usage_history], [50, 75, 90, 95])
        ...
        storage_usage_trend = np.polyfit(
            range(len(usage_history)), [u.storage_usage for u in usage_history], 1
        )[0]
        ...
        if constraints.get("allow_spot_instances", False):
            ...
            if avg_duration < 60:  # Less than 60 minutes average
                ...
                optimized_config["spot_max_price"] = constraints.get(
                    "spot_max_price", current_config.get("on_demand_price", 0.05) * 0.7
                )
    ...
    def _get_smaller_instance(self, current_type: str) -> str:
        ...
        return instance_families.get(self.cloud_provider, {}).get(current_type, current_type)
    ...
    def _estimate_workload_duration(self, usage_history: List[ResourceUsage]) -> float:
        ...
        if high_usage_periods:
            return float(np.mean(high_usage_periods) * 5)  # Assuming 5-minute intervals
        return 30.0  # Default 30 minutes

    def generate_cost_report(
        self, usage_data: List[ResourceUsage], output_path: str = "cost_report.html"
    ):
        """
        ...
        fig.add_trace(go.Scatter(y=cost_history, name="Daily Cost ($)"), row=2, col=1)
...
def main():
    ...
    explainer = ModelExplainer(
        model=model, feature_names=list(X.columns), class_names=["Class 0", "Class 1"]
    )

    # Global explanations
    global_exp = explainer.explain_global(X_train, y_train, method="shap")
    print("\nGlobal Feature Importance:")
    for feat, imp in sorted(global_exp.global_importance.items(), key=lambda x: x[1], reverse=True):
        print(f"  {feat}: {imp:.4f}")
    ...
    if local_exp.local_explanations:
        for feat, contrib in list(local_exp.local_explanations[0]["feature_contributions"].items())[
            :3
        ]:
    ...
    optimized = optimizer.optimize_resources(
        current_config, usage_data, {"target_cpu_utilization": 70, "allow_spot_instances": True}
    )
