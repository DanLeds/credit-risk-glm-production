...
class PerformanceMetrics:
    ...
    def calculate_statistics(self):
        ...
        self.p95_response_time_ms = sorted_times[p95_index] if p95_index < len(sorted_times) else self.max_response_time_ms
        self.p99_response_time_ms = sorted_times[p99_index] if p99_index < len(sorted_times) else self.max_response_time_ms
        ...
        async with aiohttp.ClientSession() as session:
            ...
            while current_time < duration_seconds:
                ...
                task = asyncio.create_task(
                    self._make_request(session, endpoint, "POST", data)
                )
                tasks.append(task)
...
class BenchmarkSuite:
    ...
    def _update_prometheus_metrics(self, test_name: str, metrics: PerformanceMetrics):
        ...
        self.throughput_gauge.labels(endpoint=endpoint).set(
    ...
    def _generate_report(self, results: Dict[str, Any]):
        ...
        for i, percentile in enumerate(['median', 'p95', 'p99']):
            values = [test['metrics'][f'{percentile}_response_time_ms'] for test in results['tests']]
...
def main():
    ...
    results = suite.run_benchmark(
        test_name="GLM Model API Performance",
